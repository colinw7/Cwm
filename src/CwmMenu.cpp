...
CwmMenuKeyData::menu_key_data_[] = {
  ...
  CwmMenuKeyData(""           , nullptr                   ),
};

bool         CwmMenu::menu_stay_up_  = false;
bool         CwmMenu::button_down_   = false;
CwmMenuItem *CwmMenu::selected_item_ = nullptr;
CwmMenu     *CwmMenu::last_menu_     = nullptr;

CwmMenuKeyData::
CwmMenuKeyData(const std::string &key, CwmMenuKeyProc proc) :
 key_(key), proc_(proc), event_(nullptr)
{
}
...
{
  auto *event1 = reinterpret_cast<XEvent *>(event);
...
CwmMenuEntry(const CwmMenuEntry &entry)
{
  ...
  if (type_ == CWM_MENU_CASCADE_TYPE) {
    auto *menu_def = static_cast<CwmMenuDef *>(entry.data1_);

    data1_ = static_cast<CwmData >(new CwmMenuDef(*menu_def));
  }
}
...
{
  if (type_ == CWM_MENU_CASCADE_TYPE) {
    auto *menu_def = static_cast<CwmMenuDef *>(data1_);
...
processNamedMenu(CwmWMWindow *window, const std::string &name)
{
  ...
  menu_def->setClientData(nullptr, window, nullptr);
...
{
  menu_def->setClientData(nullptr, nullptr, icon);
...
//}
  ...
  auto *menu = new CwmMenu(screen, nullptr, menu_def, x, y, halign, valign);
  ...
  if (menu_stay_up_) {
    ...
    while (menu->isActive()) {
      while (button_down_ && menu->isActive() &&
             menu->current_ && menu->current_->isCascadeMenu())
      ...
      if      (event->type == ButtonPress) {
        ...
        if (button_down_) {
          ...
          if (menu_item && ! menu_item->isCascadeMenu())
            selected_item_ = menu_item;
        }
      }
        ...
        XKeyPressedEvent *event1 = reinterpret_cast<XKeyPressedEvent *>(event);
        ...
        XKeyReleasedEvent *event1 = reinterpret_cast<XKeyReleasedEvent *>(event);
...
{
  if (menu->current_ && menu->current_->isCascadeMenu())
    menu->current_->processCascadeMenu(menu);
  else
    menu->nextItem();
}
 ...
 screen_(screen), parent_(parent)
{
  ...
  auto *title_color = screen.getColor(fg, bg);

  auto *title_font = CwmResourceDataInst->getMenuTitleFont(screen);
  ...
  num_items_ = int(menu_def1->getSize());

  items_ = new CwmMenuItem * [uint(num_items_)];
 ...
 menu_(menu), graphics_(graphics), stipple_graphics_(stipple_graphics)
{
  ...
  if (image_name_ != "") {
    ...
    if (image_ && (image_->getWidth() > 16 || image_->getHeight() > 16))
...
isCascadeMenu()
{
  return (type_ == CWM_MENU_CASCADE_TYPE && data1_);
}
...
processCascadeMenu(CwmMenu *parent)
{
  ...
  auto *menu = new CwmMenu(parent->getScreen(), parent, menu_def, root_x, root_y,
    ...
    while (menu->isActive()) {
      ...
      if      (event->type == ButtonPress) {
        ...
        if (CwmMenu::isButtonDown()) {
          ...
          if (menu_item) {
            ...
            if      (menu_item->menu_ == menu) {
              if (menu_item && menu_item->type_ != CWM_MENU_CASCADE_TYPE)
                CwmMenu::setSelectedItem(menu_item);
            }
            else if (menu_item->menu_) {
              if (menu_item != this) {
                CwmMenu *menu1 = menu;

                while (menu1 && menu1 != menu_item->menu_) {
                  menu1->setInactive();

                  menu1->setCurrent(nullptr);
        ...
        if (CwmMenu::isButtonDown()) {
          ...
          CwmMenu::setSelectedItem(nullptr);
        }
      }
      else if (event->type == KeyPress) {
        XKeyPressedEvent *event1 = reinterpret_cast<XKeyPressedEvent *>(event);
...
enter()
{
  if      (type_ == CWM_MENU_BUTTON_TYPE && proc_) {
  ...
  else if (type_ == CWM_MENU_CASCADE_TYPE && data1_) {
    menu_->setCurrent(this);
...
isAcceleratorEvent(XKeyPressedEvent *event)
{
  ...
  return accelerator_event_->matchEvent(reinterpret_cast<XEvent *>(event));
}
...
drawText(CwmGraphics *graphics1)
{
  ...
  for (i = 0; i < uint(text_.size()); i++)
    if (text_[i] == mnemonic_)
      break;

  if (i < uint(text_.size())) {
    int x1 = x;
...
{
  auto pnamed_menu1 = named_menus_.begin();
  auto pnamed_menu2 = named_menus_.end  ();
